...
public class GridDividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mHorizontalDividerOffsets.indexOfKey(adapterPosition) < 0) {
            mHorizontalDividerOffsets.put(adapterPosition, getHorizontalDivider(parent, adapterPosition).getIntrinsicWidth());
        }
        ...
        if (mOrientation == GRID_DIVIDER_VERTICAL) {
            int column = (adapterPosition) % spanCount + 1;
            int horDividerWidth = getHorizontalDivider(parent, adapterPosition).getIntrinsicWidth();
            outRect.left = (column - 1) * horDividerWidth / spanCount;
            outRect.right = (spanCount - column) * horDividerWidth / spanCount;
            outRect.bottom = mVerticalDividerOffsets.get(adapterPosition);

            if (isLastRow(adapterPosition, spanCount, childCount)) {
                outRect.bottom = 0;
            }
        } else {
            int row = (adapterPosition) % spanCount + 1;
            int verDividerHeight = getVerticalDivider(parent, adapterPosition).getIntrinsicHeight();
            outRect.top = (row - 1) * verDividerHeight / spanCount;
            outRect.bottom = (spanCount - row) * verDividerHeight / spanCount;
            outRect.right = mHorizontalDividerOffsets.get(adapterPosition);
    ...
    public void drawHorizontalDividers(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            mHorizontalDividerOffsets.put(params.getViewAdapterPosition(), divider.getIntrinsicWidth());
